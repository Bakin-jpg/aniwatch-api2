...
def scrape_anime_details(detail_url):
    ...
    description_el = soup.select_one('.anisc-detail .film-description .text')
    synopsis = description_el.text.strip().replace('... + More', '...').strip() if description_el else "No synopsis available."
    ...
    for ep_item in episode_items:
        episodes.append({
            ...
            'watch_url': f"{BASE_URL}{ep_item['href']}"
        })
